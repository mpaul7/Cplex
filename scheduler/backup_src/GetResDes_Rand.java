...
public class GetResDes_Rand {
	...
	public GetResDes_Rand(int nMD){
		...
		/*if (nMD < 12){
			int temp_nMD = 12;
			MDResDes = new double[8][temp_nMD];
		}else {MDResDes = new double[8][nMD];}*/
	...
	public GetResDes_Rand(int nMD, int nSN, double[][]MDResDes){
		this.nSN = nSN;
		this.nMD = nMD;
	...
	void setLatencyRemote(double[][] array, int nMD, int nSN){
		for(int i = 0; i < nMD; i++){
		for(int j = 0; j < nSN; j++ ){
			latencyRemote[j][i] = array[3][i] + array[4][i] + array[4][i];
			}
		}
	}
  ...
  */ //=============================================
  ...
  public static void main(String args[]){
	  GetResDes_Rand obj = new GetResDes_Rand(2);
