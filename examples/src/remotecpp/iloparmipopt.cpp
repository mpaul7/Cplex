...
static ParamValue const primalbound[] = {
   ParamValue(IloCplex::HeurFreq, 1), // Run heuristics at each node.
   ParamValue(IloCplex::RINSHeur, 1), // Run RINS every two nodes
   ParamValue(IloCplex::CutPass, -1), // Disable cuts
   ParamValue()
};
/** Parameter settings to have CPLEX work exclusively on the dual bound. */
static ParamValue const dualbound[] = {
   ParamValue(IloCplex::HeurFreq, -1), // Heuristics off.
   // All cuts aggressive.
   ParamValue(IloCplex::Cliques, 3),
   ParamValue(IloCplex::Covers, 3),
   ParamValue(IloCplex::DisjCuts, 3),
   ParamValue(IloCplex::FlowCovers, 2),
   ParamValue(IloCplex::FracCuts, 2),
   ParamValue(IloCplex::GUBCovers, 2),
   ParamValue(IloCplex::ImplBd, 2),
   ParamValue(IloCplex::MIRCuts, 2),
   ParamValue(IloCplex::ZeroHalfCuts, 2),
   ParamValue(IloCplex::MCFCuts, 2),
   ParamValue()
};
