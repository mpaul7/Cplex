...
main (int  argc, char *argv[])
{
   ...
   if ( status )  {
      fprintf (stderr,
             "Failed to set CPX_PARAM_MIPINTERVAL, status = %d.\n", status);
      goto TERMINATE;
   }
                     ...
                     int separate_fractional_solutions)
{
   ...
   status = CPXsetintparam (user_cbhandle->env, CPX_PARAM_REDUCE, 0);
   if ( status ) {
      fprintf(stderr, 
         "Failed to set CPX_PARAM_REDUCE, status = %d.\n", status);
      goto TERMINATE;
   }
...
set_benders_callback  (CPXENVptr env, USER_CBHANDLE *user_cbhandle)
{
   ...
   if ( status )  {
      fprintf (stderr,
             "Failed to set CPX_PARAM_MIPCBREDLP, status = %d.\n", status);
      goto TERMINATE;
   }
   ...
   if ( status )  {
      fprintf (stderr,
             "Failed to set CPX_PARAM_THREADS, status = %d.\n", status);
      goto TERMINATE;
   }
   ...
   if ( status )  {
      fprintf (stderr,
             "Failed to set CPX_PARAM_MIPSEARCH, status = %d.\n", status);
      goto TERMINATE;
   }
